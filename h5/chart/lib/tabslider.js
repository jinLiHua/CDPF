	...
	app.TabSlider = function(dom,options){
		...
		//判定事件
			...
			_touchEnd=function() {
				...
				if ($($bd).parent().parent().attr("id") == "tabSlider1"){

					$(".nums").text(currentIndex+1)
				}

				if ($($bd).parent().parent().attr("id") == "tabSlider2"){
					var arr = ['残疾类别统计图','调查状态统计图','残疾等级统计图','残疾类别统计表'];
					$(".numsYop").text(currentIndex+1);
					$(".numTitle").text(arr[currentIndex]);
				}
			},
		// hd 单击事件
		...
		//设置内容区域高度
			_contentHeight = function(){
				console.info($bd.children('div.tabSlider-box').eq(currentIndex).height())
